...
bool CreateGdiResources(int fontHeight) {
    ...
    g_Font = CreateFontW(-fontHeight, 0, 0, 0, FW_NORMAL, FALSE, FALSE, FALSE,
...
bool Draw(const std::wstring& text, float x, float y, float right, float height,
    ...
    const float drawX = (format & DT_CENTER) != 0 ? x + (right - x - drawWidth) * 0.5f : x;
