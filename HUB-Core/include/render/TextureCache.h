...
inline bool HasImageResource(const std::string& key) {
    if (key.empty()) return false;
    std::string localPath = RoleConfig::ResolveLocalPath(key);
    if (!localPath.empty() && RoleConfig::FileExists(localPath)) return true;
    if (RoleConfig::FileExists(key)) return true;
