...
bool OVPlaybackManager::Initialize()
{
    ...
    bass_.ChannelSetVolume(stream_, masterVolume_);
...
void OVPlaybackManager::SetMasterVolume(float volume)
{
    ...
    if (stream_ != 0) bass_.ChannelSetVolume(stream_, masterVolume_);
}
...
BassDword OV_BASS_CALL OVPlaybackManager::OnStream(BassHandle, void* buffer, BassDword length, void* user)
{
    ...
    if (bytes > 0) std::memcpy(buffer, playback->queue_.data() + playback->readOffset_, bytes);
